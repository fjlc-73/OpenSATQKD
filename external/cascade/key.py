...
class Key:
    ...
    def __init__(self, bitstring=None): #MODIFIED
        """
        Create a key either with an empty key (default) or from a provided bitstring.

        Args:
            bitstring (str, optional): The bitstring to initialize the key. Should be a string
            of '0' and '1' characters. If None, an empty key is created.
        """
        if bitstring:
            self._size = len(bitstring)
            self._bits = {i: int(bitstring[i]) for i in range(self._size)}
        else:
            self._size = 0
            self._bits = {}
